...
# Calculate sleep interval (seconds per change)
...
while true; do
  ...
  docker exec -i "$MYSQL_CONTAINER" \
    ...
    -e "INSERT INTO orders (product_id, quantity, order_date) VALUES ($PROD_ID, $QTY, '$DATE');"
  ...
  if (( i % 3 == 0 )); then
    ...
    docker exec -i "$MYSQL_CONTAINER" \
      ...
      -e "UPDATE orders SET quantity=$NEW_QTY WHERE order_id=$ID;"
